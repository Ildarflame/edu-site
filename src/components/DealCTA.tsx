...
import { useClaimCount } from "@/hooks/useClaimCount";
...
export default function DealCTA({ url, slug, category, audiences }: Props) {
  const { count, increment } = useClaimCount(slug);
  ...
  return (
    <div>
      <a
        ...
        onClick={() => {
          track("deal_click", { slug, category, audience: audiences.join(",") });
          increment();
        }}
